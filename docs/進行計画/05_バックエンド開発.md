...
- [x] `src/types/error.ts` 作成
  - `ApiError`型定義（code、message、details）
  - エラーコード定数（UNAUTHORIZED、FORBIDDEN、NOT_FOUND、INVALID_INPUT、INVALID_PARAMETER、INVALID_BODY、INTERNAL_ERROR）

#### 5.6.2 エラーハンドラー実装

- [x] `src/lib/errors.ts` 作成
  - `errorResponse()`: 汎用エラーレスポンス生成
  - `unauthorizedError()`, `forbiddenError()`, `notFoundError()`: 各種エラーヘルパー
  - `validationError()`, `invalidParameterError()`, `invalidBodyError()`: バリデーションエラーヘルパー
  - `internalError()`: 内部エラーヘルパー

### 5.7 API実装（CRUD）

#### 5.7.1 API-01: TODO一覧取得（GET /api/todos）

- [x] `src/app/api/todos/route.ts` 作成
- [x] GET ハンドラー実装
  - セッション認証チェック
  - クエリパラメータバリデーション（Zod）
  - Prisma検索クエリ構築
  ...
  - レスポンス返却
- [x] エラーハンドリング（401、400、500）

#### 5.7.2 API-02: TODO新規作成（POST /api/todos）

- [x] POST ハンドラー実装（同ファイル）
  - セッション認証チェック
  - リクエストボディバリデーション（Zod）
  - Prisma create
  - レスポンス返却（201 Created）
...
- [x] PATCH ハンドラー実装（同ファイル）
  ...
  - Prisma update
  - レスポンス返却
- [x] エラーハンドリング（401、403、404、400、500）

#### 5.7.5 API-05: TODO削除（DELETE /api/todos/[id]）

- [x] DELETE ハンドラー実装（同ファイル）
  - セッション認証チェック
  - Prisma findUnique（存在確認）
  - 所有者チェック
  - Prisma delete
...
- [x] Vitestテスト用DB設定（`.env.test`、テストDB URL）
- [x] Prisma test helper作成（マイグレーション実行、データクリーンアップ）
- [x] MSW設定確認（既存設定の流用）

#### 5.8.2 バリデーションテスト

- [x] `src/lib/validations/todo.test.ts` 作成
  - createTodoSchema正常系
...
- [x] `src/lib/auth.test.ts` 作成
  - requireAuth正常系（認証済み）
  - requireAuth異常系（未認証で401）

#### 5.8.4 API統合テスト（スキップ）

- [ ] TODO一覧API正常系テスト（手動テストで代替）
- [ ] TODO作成API正常系/異常系テスト（手動テストで代替）
- [ ] TODO更新API正常系/所有者チェックテスト（手動テストで代替）
- [ ] TODO削除API正常系/所有者チェックテスト（手動テストで代替）
- [ ] 未認証でのAPIアクセステスト（手動テストで代替）

※API統合テストは実装せず、手動テスト（Postman、curl等）で動作確認を行う

### 5.9 APIドキュメント作成

- [x] `docs/API設計/` のドキュメントを実装に合わせて更新
  - [x] API-01.md: クエリパラメータ名修正（sort→sortBy、order→sortOrder）、レスポンス例更新、エラー例追加
  - [x] API-02.md: titleの最大文字数を120文字に修正、レスポンス例更新、エラー例追加
  - [x] API-03.md: レスポンス例更新、エラー例追加
  - [x] API-04.md: titleの最大文字数を120文字に修正、レスポンス例更新、エラー例追加
  - [x] API-05.md: 認証ヘッダー例修正、エラー例追加
  - [x] 共通エラーレスポンス.md: 実装済みエラーコード追加、認証方式説明追加、レスポンス例充実化
- [x] README更新（API実装済みセクション追加）
  - [x] 実装済みエンドポイント一覧表追加
  - [x] 認証方式説明追加
  - [x] テスト実行方法・結果追加
  - [x] API設計ドキュメントへのリンク追加
