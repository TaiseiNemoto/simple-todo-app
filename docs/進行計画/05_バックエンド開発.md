...
- [ ] `docker-compose.yml` 作成
  ...
  - ボリューム設定（データ永続化）
- [ ] `.env.example` にDATABASE_URL追加
- [ ] `.env` にDATABASE_URL設定（ローカル環境用）
- [ ] Docker起動確認（`docker compose up -d`）
- [ ] MySQL接続確認

#### 5.1.2 Prisma初期化

- [ ] Prismaスキーマファイル確認（`prisma/schema.prisma`）
- [ ] datasource設定確認（MySQL、DATABASE_URL）
...
- [ ] `Todo`モデル作成
  ...
  - `createdAt` (DateTime, @default(now()))
  - `updatedAt` (DateTime, @updatedAt)
- [ ] リレーション定義（`user User @relation`, onDelete: Cascade）
- [ ] Enum定義（`TodoStatus`, `TodoPriority`）

#### 5.2.3 インデックス設計実装

- [ ] User: `@@unique([githubId])`（自動）
- [ ] Todo: `@@index([userId, updatedAt])`
- [ ] Todo: `@@index([userId, status])`
- [ ] Todo: `@@index([userId, priority])`
- [ ] Todo: `@@index([userId, due])`
- [ ] Todo: FULLTEXT index検討（MySQL 8.0の場合は`@@fulltext([title, description])`）

### 5.3 DBマイグレーション実行

- [ ] Prismaクライアント生成（`npx prisma generate`）
- [ ] 初回マイグレーション実行（`npx prisma migrate dev --name init`）
- [ ] マイグレーション成功確認
- [ ] Prisma Studio起動確認（`npx prisma studio`）
- [ ] 空のUserテーブル、Todoテーブル確認
