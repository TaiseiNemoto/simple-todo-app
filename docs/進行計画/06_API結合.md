...
- [ ] `src/lib/api/client.ts` 作成
  - [ ] `apiClient()` 関数実装
    - ベースURL設定（開発環境では`http://localhost:3000`）
    - JSONレスポンス自動パース
    - HTTPステータスコードチェック
  - [ ] エラーハンドリング
    - 401: 未認証エラー → サインイン画面へリダイレクト
    - 403: 権限エラー → エラーメッセージ表示
    - 404: Not Found → エラーメッセージ表示
    - 500: サーバーエラー → エラー画面へ遷移
...
- [ ] ミドルウェア作成・拡張（`src/middleware.ts`）
  - [ ] 未認証時の `/todos` アクセス → `/signin` リダイレクト
  - [ ] 認証済み時の `/signin` アクセス → `/todos` リダイレクト
  - [ ] パブリックパス設定（`/api/auth/*`）

#### 6.4.2 サインアウト実装

- [ ] `src/app/todos/page.tsx` 修正
  - [ ] サインアウトボタンのハンドラー実装
  - [ ] `signOut()` 関数呼び出し（next-auth）
  - [ ] サインアウト後 `/signin` へリダイレクト
